# ssm_v0.2 - simplified solar modeller
...
def redlineImage(im):
    ...
    pix = im.load()
    threshold = int(input('Image edge detection threshold: \n1-700, low values for white buildings: '))
    for x in range(width):
        for y in range(height - 1):
            (R, G, B) = pix[x, y]
            if R + G + B > threshold:
...
fisheye_or_pano = int(input('Input hemispherical fisheye or 360 deg panorama? '
                        '\n Enter 1 for fisheye, 2 for panorama, '
                        '\n Or anything else to terminate: '))

if fisheye_or_pano == 1:
    path_Image = easygui.fileopenbox(msg='Select skyline fisheye')
...
elif fisheye_or_pano == 2:
    path_Image = easygui.fileopenbox(msg='Select skyline panorama')
...
img_resized.save('ResizedImage.bmp', img_resized.format)

r, imgcsv_azimuth, imgcsv_elevation = redlineImage(img_resized)


# Write to csv
with open('ImageVectorData.csv', 'w', newline='') as img_csv_file:
    writer = csv.writer(img_csv_file)
    writer.writerow(['azimuth','elevation'])
    writer.writerows(zip(imgcsv_azimuth, imgcsv_elevation))

r.show()
r.save('TracedImage.bmp')
...
plt.ylabel('Skyline & sun path chart')
...
frequency = int(input('Computation interval in minutes: '))
...
text_data = open(r"RawData.txt","w")
...
print('\nTotal solar potential if the location is unshaded =', total_potential_insolation, 'watts per square meter', file=text_data)
print('\nActual solar energy accounting for shading =', total_actual_insolation, 'watts per square meter', file=text_data)
print('\nGenerating PDF report - SolarReport.pdf - file can be found in active program folder', file=text_data)

print('\nTotal solar potential if the location is unshaded =', total_potential_insolation, 'watts per square meter')
...
with PdfPages('SolarReport.pdf') as pdf:
    firstPage = plt.figure()
    firstPage.clf()
    txt1 = 'Total solar potential if the location is unshaded = ' + str(round(total_potential_insolation)) + ' watts per square meter'
    txt2 = 'Actual irradiance accounting for shading = ' + str(round(total_actual_insolation)) + ' watts per square meter'
    ...
    subprocess.run(['open', 'SolarReport.pdf'], check=True)
